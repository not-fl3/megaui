...
//         pub(super) position: Option<Point2<f32>>,
...
//         pub fn position(self, position: Point2<f32>) -> Group {
//             Group {
...
//     pub fn button1<T: Into<Option<Point2<f32>>>>(
//         &mut self,
//         pos: T,
//         size: Vector2<f32>,
//         id: Id,
...
//             |_| {},
//         );
//         *self.tabs_selected.get(&id).unwrap_or(&0)
//     }
...
//                     tabs_selected: &mut self.tabs_selected,
//                     focused: force_focus
...
//     tabs_selected: &'a mut HashMap<Id, u32>,
...
//             let size = context.ctx.measure_label(label, None);
//             let pos = pos.unwrap_or(cursor.fit(size, Layout::Vertical)) + orig;

//             context
//                 .ctx
//                 .draw_label(label, pos, None, None, Some(consts::text(context.focused)));

//             Rect::new(pos.x, pos.y, size.x as f32, size.y as f32)
//         }
//         Widget::Tabbar { pos, size, labels } => {
//             let width = size.x as f32 / labels.len() as f32;
//             let pos = pos + orig;
//             let selected = *context.tabs_selected.get(&id).unwrap_or(&0);

//             for (n, label) in labels.iter().enumerate() {
//                 let rect = Rect::new(pos.x + width * n as f32 + 1., pos.y, width - 2., size.y);
//                 let hovered = rect.contains(context.input.mouse_position);
//                 let selected = n as u32 == selected;
//                 let label_size = context.ctx.measure_label(label, None);

//                 if context.focused && hovered && context.input.click_up {
//                     *context.tabs_selected.entry(id).or_insert(0) = n as u32;
//                 }
...
//                 context.ctx.draw_label(
//                     label,
//                     pos + Vector2::new(width * n as f32 + (width - label_size.x) / 2., 0.)
//                         + Vector2::new(consts::MARGIN_BUTTON, consts::MARGIN_BUTTON + 2.),
//                     None,
//                     None,
//                     if selected {
//                         Some(Color::new(1., 1., 1., 1.))
...
//                 context.ctx.measure_label(label, None)
//                     + Vector2::new(
...
//             ..
//         }) => {
//             let pos: Point2<f32> = position.unwrap_or(cursor.fit(*size, Layout::Horizontal));
...
//         context
//             .ctx
//             .clip(Some(Rect::new(rect.x, rect.y, rect.w, rect.h)));
//     }
...
//     ctx.draw_line(p1, p2, consts::window_border(focused));
